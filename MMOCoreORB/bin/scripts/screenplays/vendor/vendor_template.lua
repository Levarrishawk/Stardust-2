...
JakkuVendorLogic = ScreenPlay:new {
	...
	currencies = {
		...
		--Displayed Name, 
		merchandise_aw = { -- Displayed name, full template string (without the shared_), cost {} - follow same order as the currencies setup previously
		...
		merchandise_as = { -- Displayed name, full template string (without the shared_), cost {} - follow same order as the currencies setup previously
...
function JakkuVendorLogic:openSUIArmorweave(pCreatureObject, pUsingObject)
	local sui = SuiListBox.new(self.scriptName, "defaultCallbackArmorweave")
	...
	sui.setTitle("Armorweave Merchandise")

	local message = "Please select which item you want to buy."
	sui.setPrompt(message)

	for i = 1, #self.merchandise_aw, 1 do
		local merchString = self:getMerchandiseStringArmorweave(i)
		sui.add(merchString, "")
	end
...
function JakkuVendorLogic:openSUIArmorsmith(pCreatureObject, pUsingObject)
	local sui = SuiListBox.new(self.scriptName, "defaultCallbackArmorsmith")
	...
	for i = 1, #self.merchandise_as, 1 do
		local merchString = self:getMerchandiseStringArmorsmith(i)
		sui.add(merchString, "")
	end
...
function JakkuVendorLogic:defaultCallbackArmorweave(pPlayer, pSui, eventIndex, args)
	...
	self:buyItemArmorweave(pPlayer, selectedOption)
end

function JakkuVendorLogic:defaultCallbackArmorsmith(pPlayer, pSui, eventIndex, args)
	local cancelPressed = (eventIndex == 1)
	...
	self:buyItemArmorsmith(pPlayer, selectedOption)
end

function JakkuVendorLogic:buyItemArmorweave(pPlayer, itemSelected)
	local merch = self.merchandise_aw[itemSelected]
	...
	local canPurchase = self:hasEnoughCurrencyArmorweave(pPlayer, itemSelected)
...
function JakkuVendorLogic:buyItemArmorsmith(pPlayer, itemSelected)
	local merch = self.merchandise_as[itemSelected]
	local pInventory = CreatureObject(pPlayer):getSlottedObject("inventory")
	local containerSize = SceneObject(pInventory):getContainerObjectsSize()
	local pGhost = CreatureObject(pPlayer):getPlayerObject()
	...
	local canPurchase = self:hasEnoughCurrencyArmorsmith(pPlayer, itemSelected)
...
function JakkuVendorLogic:getMerchandiseStringArmorweave(num)
	local merch = self.merchandise_aw[num]
...
function JakkuVendorLogic:getMerchandiseStringArmorsmith(num)
	local merch = self.merchandise_as[num]
	local merchString = merch.name .. " ("
...
function JakkuVendorLogic:hasEnoughCurrencyArmorweave(pPlayer, num)
	local pInventory = CreatureObject(pPlayer):getSlottedObject("inventory")
	local containerSize = SceneObject(pInventory):getContainerObjectsSize()
	local pGhost = CreatureObject(pPlayer):getPlayerObject()
...
function JakkuVendorLogic:hasEnoughCurrencyArmorsmith(pPlayer, num)
	local pInventory = CreatureObject(pPlayer):getSlottedObject("inventory")
	local containerSize = SceneObject(pInventory):getContainerObjectsSize()
	local pGhost = CreatureObject(pPlayer):getPlayerObject()
...
function JakkuVendorLogic:payTokensArmorweave(pPlayer, selectedItem, num)
	local pInventory = CreatureObject(pPlayer):getSlottedObject("inventory")
	local containerSize = SceneObject(pInventory):getContainerObjectsSize()
	local pGhost = CreatureObject(pPlayer):getPlayerObject()
	
	local merch = self.merchandise_aw[selectedItem]
...
function JakkuVendorLogic:payTokensArmorsmith(pPlayer, selectedItem, num)
	local pInventory = CreatureObject(pPlayer):getSlottedObject("inventory")
	local containerSize = SceneObject(pInventory):getContainerObjectsSize()
	local pGhost = CreatureObject(pPlayer):getPlayerObject()
	
	local merch = self.merchandise_as[selectedItem]
	local tokenCost = merch.cost[num]
	deleteItems = 0
	tokens = 0
	screenPlayTokenCost = 0

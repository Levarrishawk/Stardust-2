...
int ForceHealQueueCommand::runCommand(CreatureObject* creature, CreatureObject* targetCreature) const {
			...
			for (int j = 0; j < 3; j++) {
				if (totalCost < currentForce) {
					...
					if (woundAmount > 0) {
						...
						if (creature->getHAM(CreatureAttribute::ACTION) > 600){
	...
	if (healPerformed) {
		...
		} else if(creature->getHAM(CreatureAttribute::ACTION) > 600){
			playerObject->setForcePower(currentForce - totalCost);
			creature->inflictDamage(creature, CreatureAttribute::ACTION, 450, true, true, true);
		} else {
			//creature->sendSystemMessage("You do not have enough Action Points to use that ability.");
			//creature->playMusicMessage("sound/ui_quest_spawn_escort.snd");
			return GENERALERROR;
